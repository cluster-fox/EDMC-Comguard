...
from comguard.constants import FOLDER_BACKUPS, FOLDER_UPDATES, RequestMethod
from comguard.debug import Debug
from comguard.requestmanager import ComguardRequest

BACKUPS_KEEP = 3
DATETIME_FORMAT = "%Y-%m-%d-%H-%M-%S"
FILE_LATEST = "latest.zip"
FILE_DISABLE = "disable-auto-update.txt"
URL_PLUGIN_VERSION = "https://api.github.com/repos/cluster-fox/EDMC-Comguard/releases/latest" # Doesn't include pre-releases or draft releases
...
class UpdateManager:
    ...
    def __init__(self, comguard):
        ...
        self.updates_folder:str = path.join(self.Comguard.plugin_dir, FOLDER_UPDATES)
        self.backups_folder:str = path.join(self.Comguard.plugin_dir, FOLDER_BACKUPS)
        ...
        if path.exists(path.join(self.Comguard.plugin_dir, FILE_DISABLE)):
        ...
        self.Comguard.RequestManager.queue_request(URL_PLUGIN_VERSION, RequestMethod.GET, callback=self._version_info_received)


    def _version_info_received(self, success:bool, response:Response, request:ComguardRequest):
        """
        ...
        if self.remote_version > self.Comguard.version:
            # Download the new release
            self.Comguard.RequestManager.queue_request(self.release_url, RequestMethod.GET, callback=self._download_received, stream=True)


    def _download_received(self, success:bool, response:Response, request:ComguardRequest):
        """
        ...
        if self.Comguard.Ui.frame: self.Comguard.Ui.frame.after(1000, self.Comguard.Ui.update_plugin_frame())
    ...
    def _create_backup(self):
        ...
        for root, dirs, files in walk(self.Comguard.plugin_dir):
            ...
            for file in files:
                ...
                zip_file.write(file_path, path.relpath(file_path, self.Comguard.plugin_dir))
    ...
    def _extract_latest(self):
        ...
        with ZipFile(self.latest_download_file, "r") as latest:
            latest.extractall(self.Comguard.plugin_dir)
